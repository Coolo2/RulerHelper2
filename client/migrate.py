...
async def upgrade_db(c : client_pre.Client):
    ...
    version_migrations : dict[str, list[Migration]] = {
        "10":[
            Migration("players", "name string PRIMARY KEY, location string, town string, armor integer, health integer, visited_towns integer, nickname string DEAFULT NULL, bank real, messages integer, mentions integer, duration integer, last timestamp", ["name", "location", "town", "armor", "health", "visited_towns", "bank", "messages", "mentions", "duration", "last"]),
            ...
            Migration("player_history", ["player STRING","date DATE","duration INTEGER","visited_towns INTEGER","likely_town STRING","likely_nation STRING","bank REAL","messages INTEGER","mentions INTEGER","PRIMARY KEY (player, date)"], ["player", "date", "duration", "visited_towns", "likely_town", "likely_nation", "bank", "messages", "mentions"]),
