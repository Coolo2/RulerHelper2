...
class Mod(commands.Cog):
    ...
    async def _update(self, interaction : discord.Interaction, object_type : str, old_object_name : str, new_object_name : str, discord_id : str = None):
        ...
        if object_type == "player":
            obj = self.client.world.get_player(new_object_name)
        elif object_type == "town":
            obj = self.client.world.get_town(new_object_name)
        elif object_type == "nation":
            obj = self.client.world.get_nation(new_object_name)
        ...
        if object_type == "player":
            old_duration = (await self.client.player_history_table.get_record([db.CreationCondition("player", old_object_name)], [self.client.player_history_table.attribute("duration")], order=db.CreationOrder("date", db.types.OrderDescending))).attribute("duration")

            # Update flag table
            await self.client.flags_table.delete_records([db.CreationCondition("object_type", "player"), db.CreationCondition("object_name", obj.name)]) 
            await self.client.flags_table.update_records([db.CreationCondition("object_type", "player"), db.CreationCondition("object_name", old_object_name)], [db.CreationField(self.client.flags_table.attribute("object_name"), new_object_name)])
            ...
            for record in await self.client.player_history_table.get_records([db.CreationCondition("player", new_object_name)], order=db.CreationOrder("date", db.types.OrderAscending)):
                if old_duration_at_point: old_duration_at_point = await self.client.player_history_table.get_record([db.CreationCondition("player", old_object_name), db.CreationCondition("date", record.attribute("date"), "<=")], order=db.CreationOrder("date", db.types.OrderDescending))
            ...
            for record in await self.client.player_history_table.get_records([db.CreationCondition("player", old_object_name)], order=db.CreationOrder("date", db.types.OrderAscending)):
                if not await self.client.player_history_table.record_exists(db.CreationCondition("player", new_object_name), db.CreationCondition("date", record.attribute("date"))):
                    await record.update([db.CreationField("player", obj.name)]) 
                else:
                    await record.delete()
            # if duplicates exist, merge data
            for record in await self.client.visited_towns_table.get_records([db.CreationCondition("player", old_object_name)]):
                rec = await self.client.visited_towns_table.get_record([db.CreationCondition("player", new_object_name), db.CreationCondition("town", record.attribute("town"))])
                if not rec:
                    await record.update([db.CreationField("player", obj.name)]) 
                else:
            ...
            await self.client.players_table.update_records([db.CreationCondition("name", obj.name)], [db.CreationField.add("duration", old_duration)])

            await self.client.players_table.delete_records([db.CreationCondition("name", old_object_name)])
        elif object_type == "town":
            old_duration = (await self.client.town_history_table.get_record([db.CreationCondition("town", old_object_name)], [self.client.town_history_table.attribute("duration")], order=db.CreationOrder("date", db.types.OrderDescending))).attribute("duration")

            # Update flag table
            await self.client.flags_table.delete_records([db.CreationCondition("object_type", "town"), db.CreationCondition("object_name", obj.name)]) 
            await self.client.flags_table.update_records([db.CreationCondition("object_type", "town"), db.CreationCondition("object_name", old_object_name)], [db.CreationField(self.client.flags_table.attribute("object_name"), new_object_name)])

            # Iterate over old history records. If not under new name, add it. Otherwise, remove old
            for record in await self.client.town_history_table.get_records([db.CreationCondition("town", new_object_name)], order=db.CreationOrder("date", db.types.OrderAscending)):
                old_duration_at_point = await self.client.town_history_table.get_record([db.CreationCondition("town", old_object_name), db.CreationCondition("date", record.attribute("date"), "<=")], order=db.CreationOrder("date", db.types.OrderDescending))
            ...
            for record in await self.client.town_history_table.get_records([db.CreationCondition("town", old_object_name)]):
                if not await self.client.town_history_table.record_exists(db.CreationCondition("town", new_object_name), db.CreationCondition("date", record.attribute("date"))):
                    await record.update([db.CreationField("town", obj.name)]) 
                else:
                    await record.delete()
            # If duplicate exists, merge data
            for record in await self.client.visited_towns_table.get_records([db.CreationCondition("town", old_object_name)]):
                rec = await self.client.visited_towns_table.get_record([db.CreationCondition("town", new_object_name), db.CreationCondition("player", record.attribute("player"))])
                if not rec:
                    await record.update([db.CreationField("town", obj.name)]) 
                else:
                    dur = record.attribute("duration")
            ...
            await self.client.flags_table.delete_records([db.CreationCondition("object_type", "nation"), db.CreationCondition("object_name", obj.name)]) 
            await self.client.flags_table.update_records([db.CreationCondition("object_type", "nation"), db.CreationCondition("object_name", old_object_name)], [db.CreationField(self.client.flags_table.attribute("object_name"), new_object_name)])

            # Iterate over old history records. If not under new name, add it. Otherwise, remove old
            for record in await self.client.nation_history_table.get_records([db.CreationCondition("nation", old_object_name)]):
                if not await self.client.nation_history_table.record_exists(db.CreationCondition("nation", new_object_name), db.CreationCondition("date", record.attribute("date"))):
